  ...
  async ({ event, step, logger }) => {
    ...
    const cacheResult = await step.run("update-cache", async () => {
      ...
      try {
        ...
        const cacheData = {
          ...
          contributions: {
            last_year_total: stats.contributions?.last_year_total || 0,
            current_streak: stats.contributions?.current_streak || 0,
            longest_streak: stats.contributions?.longest_streak || 0,
          },
    ...
    return {
      ...
      stats: {
        ...
        followers: profile.followers,
        contributionStreak: stats.contributions?.current_streak || 0,
      },
